...
export default function ChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const fallbackResponse: Message = {
        id: (Date.now() + 1).toString(),
        content: "I apologize, but I'm having trouble connecting right now. Please try again later or contact a healthcare professional for immediate assistance.",
        role: "assistant",
        timestamp: new Date(),
        isEmergency: false,
      }
  ...
  return (
    <ProtectedRoute allowedRoles={["patient"]} loginRoute="/patient/login">
      ...
      {/* Header */}
      <header className={`border-b backdrop-blur-sm px-6 py-4 ${
        ...
        <div className={`border-l-4 border-red-500 p-4 ${
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              }`}>Emergency Detected!</p>
              <p className={`text-sm ${
                theme === 'dark' ? 'text-red-400' : 'text-red-700'
              }`}>Please call emergency services immediately: 911</p>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className={`text-lg ${
                      theme === 'dark' ? 'text-zinc-100' : 'text-zinc-900'
                    }`}>Hakmin AI Assistant</CardTitle>
                    <p className={`text-sm ${
                      theme === 'dark' ? 'text-zinc-400' : 'text-zinc-600'
                    }`}>Your AI health companion powered by ChatGPT</p>
                  </div>
                    ...
                    >
                      <div className={`flex items-start space-x-2 max-w-[80%] ${message.role === "user" ? "flex-row-reverse space-x-reverse" : ""}`}>
                        <Avatar className="w-8 h-8">
                          <AvatarFallback className={message.role === "user" ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white" : "bg-gradient-to-r from-sky-400 to-blue-600 text-white"}>
